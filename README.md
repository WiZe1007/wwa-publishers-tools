...
`public/js/backdrop.js` підбирає одну версію за розміром екрана та DPR. Файли
`mountains-*-v2` створені з оригінального `319751.mp4`, а не з попередньої 720p-копії.
Усі версії зберігають 60 fps, не містять аудіо та мають MP4 fast-start.

| Версія | Роздільність | Кодування | Розмір |
|---|---|---|---|
| Desktop | 1920×1080 | x264 slow, CRF 21 | 12,0 МБ |
| Mobile | 1080×1920 | x264 slow, CRF 22 | 14,9 МБ |
| Ultra | 3840×2160 | x265 medium, CRF 22, hvc1 | 49,0 МБ |
| Ultra fallback | 3840×2160 | x264 slow, CRF 23 | 52,5 МБ |

Для портретного кадру використано crop `1216:2160:1152:0` перед масштабуванням.
На 3G вибирається Full HD замість 4K; Save-Data, 2G та reduced motion вимикають
автозапуск. Пауза, кешування та один відеоелемент зберігаються між сторінками.
